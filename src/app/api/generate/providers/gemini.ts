...
/** Gemini safety categories a blanket threshold is applied to. */
...
/** UI safety level → Gemini HarmBlockThreshold. */
const SAFETY_THRESHOLD: Record<string, string> = {
  none: "BLOCK_NONE",
...
export async function generateWithGemini(
  ...
  // Advanced parameters (Nano Banana 2 / Pro) carried in the node's `parameters`.
  const p = parameters || {};
  // Seed — reproducible results.
  if (p.seed !== undefined && p.seed !== null && p.seed !== "") {
    const seedNum = Number(p.seed);
    if (Number.isFinite(seedNum)) config.seed = Math.trunc(seedNum);
  }
  // System instruction.
  if (typeof p.systemPrompt === "string" && p.systemPrompt.trim()) {
    config.systemInstruction = p.systemPrompt.trim();
  }
  // Number of images → candidateCount (1–4).
  const numImages = Number(p.numImages);
  if (Number.isFinite(numImages) && numImages > 1) {
    config.candidateCount = Math.min(4, Math.max(1, Math.round(numImages)));
  }
  // Safety thresholds (blanket level applied to all categories).
  if (typeof p.safety === "string" && p.safety !== "default" && SAFETY_THRESHOLD[p.safety]) {
    config.safetySettings = SAFETY_CATEGORIES.map((category) => ({ category, threshold: SAFETY_THRESHOLD[p.safety as string] }));
  }
  // Thinking level (model-dependent).
  if (typeof p.thinkingLevel === "string" && p.thinkingLevel !== "default" && p.thinkingLevel) {
    config.thinkingConfig = { thinkingLevel: p.thinkingLevel };
  }
  ...
  // Collect one image per candidate (candidateCount > 1 ⇒ multiple images via
  // the "Number of images" option). The executor stores all of them in history.
  ...
  for (const cand of candidates) {
    ...
    for (const part of parts) {
      if (part.inlineData && part.inlineData.data) {
        ...
        allImages.push(`data:${mimeType};base64,${part.inlineData.data}`);
        break;
      }
    }
  }
  ...
  if (allImages.length > 0) {
    const responsePayload = { success: true, image: allImages[0], images: allImages };
    const responseSize = JSON.stringify(responsePayload).length;
    const responseSizeMB = (responseSize / (1024 * 1024)).toFixed(2);
    if (responseSize > 4.5 * 1024 * 1024) {
      console.warn(`[API:${requestId}] Response size (${responseSizeMB}MB) approaching Next.js 5MB limit (${allImages.length} images)`);
    }
    console.log(`[API:${requestId}] SUCCESS - Returning ${allImages.length} image(s), ${responseSizeMB}MB`);
