...
- The generator regenerates the image FRESH from your prompt + the references every turn — it does NOT edit the render. So the prompt must FULLY specify the desired image: RE-ASSERT the full ORIGINAL spec every turn (each reference's role, required counts, must-nots) PLUS this turn's targeted fixes PLUS anything from the render worth keeping (described IN WORDS, since the generator can't see the render). Do NOT shrink to just the incremental tweak, and do NOT carry over drift — hold to the original intent. State the hard constraints explicitly every time (e.g. "use Image 1 ONLY for composition, framing, layout and palette — not its painterly surface; render as a real modern photograph, not a painting").
